...
class NormLinearAttention(nn.Module):
    ...
    def forward(self, x, context, additional_tokens=None, n_times_crossframe_attn_in_self=0):
        ...
        if self.forward_type == 'left':
            return self.forward_left(q, k, v, u)
        elif self.forward_type == 'right':
            return self.forward_right(q, k, v, u)
...
class BasicTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        if attn_mode == "softmax-xformers" and not XFORMERS_IS_AVAILABLE:
            attn_mode = "softmax"
            print(
    ...
    def _forward(
        ...
        if self.disable_self_attn:
            arg_context = context
            arg_n_times_crossframe_attn_in_self = 0
        else:
            arg_context = None
            arg_n_times_crossframe_attn_in_self = n_times_crossframe_attn_in_self
        ...
        x = (
            self.attn1(
                ...
                n_times_crossframe_attn_in_self=arg_n_times_crossframe_attn_in_self,
            )
            + x
        )
        x = (
