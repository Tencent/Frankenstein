...
class FPN_down_g(nn.Module):
    def __init__(self, in_channel_list, out_channel_list):
        ...
        for i, in_channel in enumerate(in_channel_list[:-1]):
            ...
            # self.inner_layer.append(nn.Conv2d(in_channel, out_channel_list[i], 1).cuda())
            # self.out_layer.append(nn.Conv2d(out_channel_list[i], out_channel_list[i], kernel_size=3, padding=1).cuda())
        # self.upsample=nn.Upsample(size=, mode='nearest')
    ...
    def forward(self, x):
        ...
        for i in range(len(x) - 1):
            ...
            # print(prev_feature.shape, current_feature.shape)
            # raise RuntimeError
...
if __name__ == '__main__':
    ...
    features_down_dim = [512, 1024, 1024, 1024]
    features_out_dim = [512, 1024, 1024]

    features_size = [32, 16, 8, 4]
    features_down = []
    for i in range(len(features_size)):
        
        feature = torch.rand(1, features_down_dim[i], features_size[i], features_size[i]*3).cuda()
        print(feature.shape)
        features_down.append(feature)
    
    fpn_up = FPN_up(features_down_dim[::-1], features_out_dim[::-1])

    features_output = fpn_up(features_down[::-1])
